...
def update_actor(ident: int, film_id: int, actor_id: int, db: Session = Depends(database),
              current_user: Users = Depends(get_current_user)):
    if current_user.role != "admin":
        raise HTTPException(403, "You are not authorized to update an actor")
    db.query(FilmActor).filter(FilmActor.id == ident).update({
...
def delete_actor(ident: int, db: Session = Depends(database),
              current_user: Users = Depends(get_current_user)):
    if current_user.role != "admin":
        raise HTTPException(403, "You are not authorized to delete an actor")
    db.query(FilmActor).filter(FilmActor.id == ident).delete()
    db.commit()
